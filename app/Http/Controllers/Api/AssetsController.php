    ...
    public function index(Request $request, $audit = null) 
    {
        ...
        $assets = Company::scopeCompanyables(Asset::select('assets.*'),"company_id","assets")
            ->with('location', 'assetstatus', 'assetlog', 'company', 'defaultLoc','assignedTo',
                'model.category', 'model.manufacturer', 'model.fieldset','supplier','process');
;
        ...
        switch ($request->input('status')) {
            ...
            case 'Pending':
                ...
                })->join('process_details as p', function ($join){
                    $join->on('p.process_id','=','assets.process_id')
                         ->on('p.level','=','assets.level')
            ...
            case 'Archived':
                ...
                $assets->where('assets.assigned_to', '>', '0')
                        ->where('assets.assign_level', '>', '0')
                        ->join('process_details as p', function ($join){
                            $join->on('p.process_id','=','assets.process_id')
                                 ->on('p.level','=','assets.assign_level')
                                 ->where('p.user_id','=',Auth::user()->id);
                        });
                    break;
            case 'Requestable':
